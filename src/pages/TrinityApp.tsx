...
export default function TrinityApp() {
  ...
  const getFanDeckConfig = (variant: "page" | "modal", compact: boolean) => {
    if (compact) {
      return variant === "page"
        ? {
            ...
            heightClass: "h-52",
            cardClass:
              "absolute w-[3.75rem] h-[6.25rem] bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-950 border border-yellow-500/30 rounded-2xl shadow-xl flex items-center justify-center cursor-pointer hover:border-yellow-400 hover:shadow-[0_0_20px_rgba(234,179,8,0.25)] transition-all active:scale-95 group/card",
        ...
        : {
            ...
            heightClass: "h-48",
            cardClass:
              "absolute w-14 h-24 bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-950 border border-yellow-500/30 rounded-2xl shadow-xl flex items-center justify-center cursor-pointer hover:border-yellow-400 hover:shadow-[0_0_20px_rgba(234,179,8,0.25)] transition-all active:scale-95 group/card",
    ...
    return variant === "page"
      ? {
          ...
          cardClass:
            "absolute w-[4.5rem] h-[7.5rem] bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-950 border border-yellow-500/30 rounded-2xl shadow-xl flex items-center justify-center cursor-pointer hover:border-yellow-400 hover:shadow-[0_0_20px_rgba(234,179,8,0.25)] transition-all active:scale-95 group/card",
      ...
      : {
          ...
          heightClass: "h-56 md:h-60",
          cardClass:
            "absolute w-16 h-28 bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-950 border border-yellow-500/30 rounded-2xl shadow-xl flex items-center justify-center cursor-pointer hover:border-yellow-400 hover:shadow-[0_0_20px_rgba(234,179,8,0.25)] transition-all active:scale-95 group/card",
  ...
  const renderDailyCardBack = (variant: "page" | "modal") => {
    ...
    return (
      <>
        ...
        <div className="absolute inset-1 border border-yellow-500/20 rounded-xl flex flex-col items-center justify-center bg-yellow-500/5 group-hover/card:bg-yellow-500/10 transition-all shadow-inner">
          <div className={`${iconWrap} rounded-full border border-yellow-500/20 flex items-center justify-center bg-black/40 shadow-md`}>
            <Sparkles size={iconSize} className="text-yellow-400 group-hover/card:scale-110 transition-all shadow-[0_0_8px_rgba(234,179,8,0.6)] animate-pulse" />
          </div>
        </div>
      </>
    );
        ...
        {shuffledTrinityCards.map((card, idx) => {
          ...
          return (
            <motion.button
              ...
              whileHover={{ y: yOffset - 28, scale: 1.12, rotateZ: rotateZ * 0.2, zIndex: 200, transition: { type: "spring", stiffness: 220, damping: 16 } }}
  ...
  const ALL_TAROT_SUGGESTIONS = [
    ...
    "오늘 하루 제가 특히 조심하거나 주목할 부분은 무엇인가요?",
  ...
  useEffect(() => {
    ...
    const loadSoulMirrorData = async () => {
      try {
        ...
        console.error("Error loading persisted soul data:", e);
      }
    };
    loadSoulMirrorData();
  }, [firebaseUser]);
    ...
    const subscribe = () => {
      ...
      unsub = onSnapshot(q, (snap) => {
        ...
        if (msg.includes('INTERNAL ASSERTION FAILED')) {
          ...
          console.error('[Trinity] onSnapshot error:', error);
          setIsHistoryLoading(false);
        }
      });
    };
...
2. 오직 질문자가 던진 구체적이고 실제적인 상황 및 질문("${tarotConcern}")에만 100% 타겟하여, 현실에 발을 붙인 명쾌한 언어로 답변하십시오.
3. 묻지 않은 배경 설명이나 뜬구름 잡는 위로, 혹은 수수께끼 같은 복잡한 설명 대신 눈앞에 놓인 팩트와 가이드라인만 단도직입적으로 서술하십시오.
4. 물리학, SF적 가식적 용어는 절대 금지입니다.
...
- 카드들의 직접적이고 솔직한 경고를 합산하여 **[확실한 YES / 조건부 YES / 절대 NO / 우회 및 보류]** 중 단 한 가지 판정을 굵은 글씨로 완전히 선언하고, 그 단도직입적인 이유를 1~2문장의 아주 직설적인 문장으로 확언하십시오.

### 📊 3단계: 성공 지수 및 현실 예측 (Realistic Success Index)
- 질문 가치와 방향성이 실현될 구체적인 **실현 지수 (0% ~ 100%)**를 수치화하여 명시하십시오.
- 상징에 근거한 구체적인 **각 카드별/통합 성공 확률과 실패 확률(예: [${cardNames}] - 성공 확률: 80% | 실패 확률: 20%)**을 명확히 기재한 후, 현실적 위험 요인과 이율을 칼날같이 묘사해 주십시오.

### 🎴 4단계: 카드 상징과 현실적 팩트 해독 (Card Symbolism & Reality Check)
- 뽑힌 ${selectedCards.length}장의 카드 [${cardNames}]를 **${concernAnalysis.spread.name}** 위치 의미에 맞춰 각각 해석하되, 은유나 신비주의적인 묘사는 배제하십시오. 
- 질문자가 마주한 현실 자각, 보이지 않는 무의식적 장벽, 그리고 직접적인 긍정/부정적 핵심 팩트를 냉철하게 파헤쳐 비추십시오.

### ⚡ 5단계: 오늘 바로 실천할 행동 계획 (Actionable Prescription)
- 흐리멍텅한 결말은 고민을 연장시킬 뿐입니다. 오늘부터 당장 상황을 진전시킬 기한 기반 계획을 명확히 지목하십시오.
  * **24시간 이내 즉시 행동 (Immediate Action within 24 Hours)**: 질문자가 지금 즉시 손쉽게 실천할 수 있는 1가지 실제적이고 명확한 액션 플랜.
  * **72시간 이내 후속 지침 (Follow-up Action within 72 Hours)**: 다음 에너지 정렬과 성공을 안착시키기 위해 3일 내 수행해야 할 후속 구체적 업무나 대화.

### 🔑 6단계: 주체적인 결단과 이정표 (Sovereign Resolution)
- 타로는 고정된 미래를 점치는 도구가 아닌, 현재 흐름의 경고와 조언입니다. 최종 열쇠와 주도권은 온전히 질문자님에게 있습니다. 가슴 깊이 남겨두어야 할 한 문장의 핵심 교훈을 단호한 어조로 가슴에 새겨 주십시오.${binaryChoicePromptAddon}${spreadPromptAddon}${contextPromptAddon}`;
        }
        ...
        <div className="max-w-5xl w-full mx-auto px-3 sm:px-6 prism-xs-pad flex-1 flex flex-col min-w-0">
          <AnimatePresence mode="wait">
            {activeMode === "tarot" ? (
                            ...
                            <div className="space-y-3 text-left w-full overflow-hidden">
                              ...
                              {tarotConcern.trim() && (
                                <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/5 px-4 py-3">
                                  <p className="text-[10px] font-bold uppercase tracking-widest text-yellow-500/80 mb-1">
                                    자동 추천 배열법
                                  </p>
                                  <p className="text-xs text-yellow-100 font-semibold">
                                    {tarotSpreadRecommendation.name} · {tarotSpreadRecommendation.cardCount}장
                                  </p>
                                  <p className="text-[10px] text-white/45 mt-1 leading-relaxed break-keep">
                                    {tarotSpreadRecommendation.reason}
                                  </p>
                                  <p className="text-[10px] text-white/35 mt-2 break-keep">
                                    {tarotSpreadRecommendation.positions.join(" → ")}
                                  </p>
                                </div>
                              )}
                            </div>
              ...
              <motion.div key="landing" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="flex-1 w-full flex flex-col items-center justify-start md:justify-center pt-6 pb-24 md:pt-16 md:pb-32 text-center lg:text-left gap-6 md:gap-12 animate-fade-in">
                <div className="w-full max-w-5xl mx-auto">
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">
                    ...
                    <div className="lg:col-span-6 w-full space-y-6 md:space-y-8">
                      ...
                      <div className="mx-auto lg:mx-0 w-full max-w-md p-5 sm:p-6 rounded-3xl bg-white/5 border border-yellow-500/20 backdrop-blur-xl flex flex-col items-center gap-4 shadow-xl hover:border-yellow-500/40 transition-all duration-300">
                        <div className="flex flex-col min-[380px]:flex-row items-start min-[380px]:items-center justify-between gap-2 w-full border-b border-white/5 pb-3">
                          <div className="flex items-center gap-2">
                            <span className="relative flex h-2 w-2">
                              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isPlayingBinaural ? 'bg-yellow-400' : 'bg-white/30'}`}></span>
                              <span className={`relative inline-flex rounded-full h-2 w-2 ${isPlayingBinaural ? 'bg-yellow-500' : 'bg-white/40'}`}></span>
                            </span>
                        ...
                        {binauralList.length > 0 && (
                          <div className="w-full mt-2 border-t border-white/5 pt-3 flex flex-col gap-2 max-h-40 overflow-y-auto no-scrollbar">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-[8px] text-yellow-400 font-mono">{binauralList.length} Tracks</span>
                            </div>
                              ...
                              return (
                                <div
                                  ...
                                  className={`flex items-center justify-between p-2 rounded-xl cursor-pointer transition-all ${
                                    isThisSelected ? 'bg-white/10 border border-yellow-500/20' : 'bg-white/[0.02] border border-transparent hover:bg-white/5'
                                  }`}
                                >
                                  <div className="flex-1 min-w-0 pr-2 text-left overflow-hidden">
                                    <BinauralTrackMarquee active={isThisTrackPlaying} text={track.name} className={`text-[11px] font-medium leading-tight ${isThisTrackPlaying ? 'text-yellow-400' : 'text-white/80'}`} />
                                  ...
                                  <div className="flex items-center gap-1 shrink-0">
                                    ...
                                    <div className={`w-6 h-6 rounded-lg flex items-center justify-center ${isThisTrackPlaying ? 'bg-yellow-500/20 text-yellow-400 animate-pulse' : 'bg-white/5 text-white/40'}`}>
                                      {isThisTrackPlaying ? (
                          ...
                          <DailyBgmSection
                            ...
                            onPersist={(patch) => {
                              setDailyResult((prev: any) => {
                                ...
                                try {
                                  const uid = firebaseUser?.uid || "guest";
                                  localStorage.setItem(getTrinityDailyResultKey(uid), JSON.stringify(next));
                                } catch (_) {}
                                return next;
                              });
                            }}
                    ...
                    <div className="space-y-6 text-left">
                      <div className="p-8 bg-yellow-500/10 border border-yellow-500/20 rounded-[48px] shadow-inner font-sans">
                        ...
                        <div className="flex justify-end mt-4">
                          <TTSButton text={insightResult.guidance} voice="Fenrir" className="shrink-0" />
                        </div>
                      </div>
